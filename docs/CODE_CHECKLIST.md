...
| 版本 | 日期 | 更新人 | 更新内容 |
|------|------|--------|----------|
| v1.7 | 2026-04-21 | Kimi | 业务逻辑全面治理：退菜校验、清台异常重构、顾客端沽清硬拦截、铁锅鱼检查统一、dining追加保持状态、金额测试+17用例、待清台筛选标签 |
...
#### BUG-GOV-001: 退菜/删菜无后端校验 - 已制作菜品可被随意删除
- **发现时间**: 2026-04-21
- **发现人**: 架构审查（业务流程梳理）
- **影响范围**: 订单编辑、金额计算、财务对账
- **严重程度**: 🔴 P0
...
##### 问题描述
系统无独立"退菜"接口，退菜通过编辑订单实现。员工在编辑页将菜品数量减至 0 或点击删除后提交，后端对 `cooking`/`cooked`/`served` 状态的菜品删除无任何校验。厨房已出餐，系统记录被删除，造成账实不符。
...
##### 修复方案
1. 后端 Hook 新增 `itemsRemoved` 检测：遍历旧 `items`，找出 `dishId` 在新 `items` 中不存在的项
2. 若被删除项的 `status` 为 `cooking`/`cooked`/`served`，抛出 `400` 错误："已制作/已上菜的菜品不可直接删除"
3. 如需退菜，走专门的退菜流程（未来可扩展）

##### 预防措施
- 检查清单新增「🟥 编辑订单必须校验删除项状态」：任何允许编辑订单的系统，后端必须校验被删除/减少的菜品是否已处于不可撤销状态
- 测试覆盖：编辑订单时尝试删除 `cooking`/`cooked`/`served` 菜品，断言后端返回 400

##### 相关提交
- 修复 PR: #biz-logic-governance-20260421
- 检查清单更新: v1.7

---

#### BUG-GOV-002: 手动清台兜底过于宽松 - 数据一致性风险
- **发现时间**: 2026-04-21
- **发现人**: 架构审查（业务流程梳理）
- **影响范围**: 清台逻辑、订单状态、桌台状态
- **严重程度**: 🔴 P0
...
##### 问题描述
`useClearTable.ts` 的 `executeClearTable` 中，订单查询失败、`completed→settled` 更新失败均静默忽略并继续清台。可能导致 `completed` 订单未归档为 `settled` 就清台，数据不一致。

##### 根因分析
1. **技术原因**：`try/catch` 块内错误被空 `catch` 吞掉，未向上抛出
2. **设计原因**：清台操作被设计为"尽力而为"，担心阻塞用户操作，但牺牲了数据一致性
3. **流程原因**：缺乏"操作失败必须告知用户"的 UX 原则

##### 修复方案
1. 订单查询失败 → 抛出错误："无法确认当前桌台订单状态，请检查网络后重试"
2. `completed→settled` 更新失败 → 抛出错误："订单状态更新失败，请稍后重试"
3. 增加兜底检查：若订单仍处于 `pending`/`cooking`/`serving`/`dining`，抛出错误阻断清台
4. 上层调用方（`OrderListView`/`OrderDetailView`）已有 `try/catch`，异常会被正确捕获并 `toast.error`

##### 预防措施
- 检查清单新增「🟥 关键业务操作禁止静默失败」：金额、状态、清台等操作失败时必须阻断并明确提示用户，禁止空 `catch` 继续执行
- 代码审查时重点检查 `try { ... } catch { /* 空 */ }` 模式
...
##### 问题描述
同一系统两个入口对同一业务规则（沽清拦截）处理截然相反：员工端是硬拦截（UI 禁用 + 添加阻断 + 提交阻断），顾客端是软拦截（允许加入购物车，提交时才自动移除）。

##### 根因分析
1. **技术原因**：顾客端 `useCart.ts` 的 `addToCart` 未检查 `dish.soldOut`
2. **设计原因**：顾客端和员工端分别由不同开发者实现，缺乏统一的业务规则文档
3. **体验考量差异**：员工端追求"不出错"，顾客端追求"不阻断"，但缺乏明确的策略决策记录

##### 修复方案
1. 统一为"硬拦截"策略：已沽清菜品在 UI 层即禁用（Stepper 按钮替换为"已沽清"标签）
2. `useCart.ts` 的 `addToCart` 增加 `soldOut` 检查：`toast.warning` 后 `return`
3. 后端 `validateItemsSoldOut` 作为最终兜底

##### 预防措施
- 检查清单新增「🟨 多端业务规则一致性」：同一业务规则（如沽清、折扣、权限）在多个前端入口必须行为一致，变更时需同步修改所有入口
- 新增业务流程图文档 `docs/BUSINESS_PROCESS_FLOW.md` 作为多端对齐依据
...
##### 问题描述
顾客端页面不刷新时，看不到菜品沽清状态变化。员工标记沽清后，顾客最长 15 秒内仍可将已沽清菜品加入购物车，提交时才被移除，体验差。

##### 根因分析
1. **技术原因**：`CustomerOrderView.vue` 未接入 `subscribeToDishes` SSE（需要认证）
2. **架构原因**：PocketBase `dishes` 集合访问规则为 `@request.auth.id != ''`，匿名用户无法订阅 SSE
3. **替代方案缺失**：无菜品轮询机制，仅依赖订单轮询（15s）

##### 修复方案
1. 新增菜品独立轮询：`useAutoRefresh(refreshDishes, { interval: 10000 })`
2. `refreshDishes()` 每 10 秒调用 `PublicDishAPI.getDishes()`，对比前后 `soldOut` 状态
3. 若购物车中的菜品变为 `soldOut`，立即 `toast.warning` 提示移除
4. 页面隐藏时自动暂停轮询，`visibilitychange` 恢复时立即刷新

##### 预防措施
- 检查清单新增「🟨 实时性要求高的数据需有主动刷新机制」：沽清、库存、价格等顾客可见的实时数据，必须有轮询/SSE/ WebSocket 等主动推送机制，不能仅依赖页面加载时的一次性获取
...
##### 问题描述
点铁锅鱼/铁锅炖鱼时自动加锅底。员工端 `OrderFormView.vue` 会检查锅底是否沽清，但顾客端 `useCart.ts` 不检查。顾客可能下单时锅底已沽清，提交时才被后端拦截。

##### 根因分析
1. **技术原因**：`useCart.ts` 的 `addToCart` 在自动加锅底逻辑中未检查 `addDish.soldOut`
2. **代码复用问题**：员工端和顾客端使用不同的 `addToCart` 实现（员工端内联在组件中，顾客端使用 `useCart` composable）

##### 修复方案
1. `useCart.ts` 中自动加锅底前检查 `addDish.soldOut`
2. 若锅底沽清：`toast.warning('配菜 "锅底" 已沽清，无法自动添加')`，不自动添加锅底
3. 铁锅鱼仍正常加入购物车（顾客可单独决定是否继续）

##### 预防措施
- 检查清单新增「🟨 自动规则（如配菜、赠品）必须统一校验」：任何自动触发的附加项（配菜、赠品、服务费）在所有前端入口必须执行相同的可用性校验
...
#### BUG-GOV-006: dining 订单追加菜品重置为 pending - 业务逻辑混乱
- **发现时间**: 2026-04-21
- **发现人**: 架构审查（业务流程梳理）
- **影响范围**: 订单编辑、状态机、厨房制作
- **严重程度**: 🟡 P1
...
##### 问题描述
订单处于 `dining`（用餐中）状态时追加菜品，后端 Hook 将订单整体状态重置为 `pending`。服务员困惑：明明客人已经在用餐了，为什么订单又变成"待确认"？厨房可能重复制作已上过的菜品。

##### 根因分析
1. **技术原因**：`pb_hooks/orders.pb.js` 中 `itemsAppended && (oldStatus === 'dining' || oldStatus === 'serving')` 时主动 `record.set('status', 'pending')`
2. **设计原因**：早期假设"追加菜品 = 全新订单"，未考虑 dining 状态追加是正常业务场景
3. **状态机理解偏差**：将"新菜品 pending"等同于"订单整体 pending"

##### 修复方案
1. 删除后端 Hook 中 `dining/serving` 追加重置为 `pending` 的代码块
2. 订单状态保持 `dining`/`serving`，仅新增菜品的 `status` 为 `pending`
3. 后端自动推断的防回退机制会阻止状态从 `dining` 回退到 `pending`
4. 无需前端确认弹窗（因为状态不再重置）

##### 预防措施
- 检查清单新增「🟨 状态变更必须有业务动因」：任何主动修改订单状态的代码（尤其是回退/重置），必须在注释中说明业务场景和决策依据
- 状态机文档 `docs/BUSINESS_PROCESS_FLOW.md` 明确标注各状态下的允许操作
...
#### BUG-GOV-007: 金额计算前后端实现存在差异 - 隐性不一致风险
- **发现时间**: 2026-04-21
- **发现人**: 架构审查（业务流程梳理）
- **影响范围**: 订单金额、折扣计算、财务对账
- **严重程度**: 🟢 P2
...
##### 问题描述
前端 `MoneyCalculator` 与后端 Hook 的金额计算代码路径不同。虽然当前测试未发现问题，但任何一方的修改都可能导致前后端金额不一致。

##### 根因分析
1. **技术原因**：前端 `MoneyCalculator.calculate()` 与后端 Hook 分别独立实现，无共享代码
2. **语言差异**：前端 TypeScript / 后端 PocketBase JS VM (goja)，无法直接共享模块
3. **测试不足**：`security.spec.ts` 仅 4 个 MoneyCalculator 测试用例，未覆盖边界场景

##### 修复方案
1. `security.spec.ts` 补充 17 个边界测试用例：浮点精度（0.1×3、0.07×7）、小数数量（1.5 份）、多件混合、百分比折扣边界（0折/10折/>10折）、固定减免边界（0元/等于总价/大于总价）
2. 测试文件顶部添加注释：「必须与后端 Hook 算法保持一致」
3. 后端 Hook 对应位置添加交叉引用注释

##### 预防措施
- 检查清单新增「🟨 金额计算变更必须双端同步测试」：任何修改金额计算逻辑的 PR，必须同时更新前端测试和后端测试（或至少在后端 Hook 注释中标注对应前端测试位置）
- 长期规划：将 MoneyCalculator 抽取为纯 JavaScript 版本，通过构建脚本同步到 `pb_hooks/`
...
#### BUG-GOV-008: completed 订单清台路径不直观 - 待清台识别困难
- **发现时间**: 2026-04-21
- **发现人**: 架构审查（业务流程梳理）
- **影响范围**: 订单列表、清台操作
- **严重程度**: 🟢 P2
...
##### 问题描述
`completed`（已结账）和 `dining`（用餐中）的桌台都显示"占用中"，订单列表快捷筛选中 `COMPLETED` 显示为"已结账"。操作者难以快速识别哪些订单需要清台，容易遗漏。

##### 根因分析
1. **术语问题**：`COMPLETED` 的 `StatusLabels` 为"已结账"，强调的是"结账完成"，而非"待清台"
2. **视觉区分不足**：所有状态筛选按钮使用相同的蓝/灰配色，无特殊标识
3. **业务场景理解**：在小型餐厅一人多岗场景下，操作者需要的是"哪些桌可以释放了"，而非"哪些账已结"

##### 修复方案
1. 订单列表快捷筛选按钮中，`COMPLETED` 显示文案改为「待清台」
2. 未选中时使用琥珀色边框+文字（`text-amber-600 border-amber-200`）
3. 选中时使用琥珀色背景+白字（`bg-amber-500 text-white`）
4. 下拉框中仍显示"已结账"（不影响详情页等场景的术语一致性）

##### 预防措施
- 检查清单新增「🟩 状态标签应考虑操作者视角」：状态展示文案应根据使用场景调整。如 `completed` 在详情页显示"已结账"，在列表筛选中显示"待清台"
- UI 设计时考虑"操作意图"而非"技术状态"
